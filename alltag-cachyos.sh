...
blau "multilib prüfen (für Steam und 32-Bit-Wine)"
if grep -qE '^\s*\[multilib\]' /etc/pacman.conf; then
    grau "ist aktiv"
else
    warn "multilib ist abgeschaltet — Steam und Teile von Wine fehlen dann"
    warn "in /etc/pacman.conf die Zeilen [multilib] und Include entsperren"
fi
...
sudo pacman -S --needed --noconfirm \
    ...
    librewolf \
    hunspell-de
gruen "Firefox, Thunderbird, Filezilla, Gimp, Gamescope, Steam, OpenRGB, Librewolf"
...
verwaist=$(pacman -Qtdq 2>/dev/null || true)
if [ -n "$verwaist" ]; then
    printf '%s\n' "$verwaist" | sudo pacman -Rns --noconfirm - >/dev/null 2>&1 || true
    gruen "ungenutzte Laufzeitumgebungen und verwaiste Pakete entfernt"
else
